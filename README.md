Lean Copilot: LLMs as Copilots for Theorem Proving in Lean (Forked Version)
==========================================================
...
> ```bash
> pip install vllm
> vllm serve bytedance-research/BFS-Prover --port 30001
> ```
>
> After setting up the model, set the `base_urls` and `api_keys` in `python/server.py`.
>
> For example:
> ```python
...
>     api_keys=["EMPTY"],  # "EMPTY" is a dummy API key for local inference servers (like vLLM)
>     ...
> ),
>```
>
> Then, launch Lean Copilot with:
>
> ```bash
> pip install fastapi uvicorn loguru openai numpy
> cd python
> python -m uvicorn server:app --port 23337
> ```
>
> Finally check if Lean Copilot and BFS-Prover are functioning correctly in `LeanCopilotTests/ModelAPIs.lean`. Set up an ExternalGenerator like this:
>
>```lean
...
2. [Using Lean Copilot in Your Project](#using-lean-copilot-in-your-project)
   ...
   2. [Getting Started with Lean Copilot](#getting-started-with-lean-copilot)
...
* Supported platforms: Linux, macOS, and [Windows WSL](https://learn.microsoft.com/en-us/windows/wsl/install); ⚠️ Native Windows currently not supported.
